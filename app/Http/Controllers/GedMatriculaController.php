...
class GedMatriculaController extends Controller
{
    ...
    SELECT DISTINCT
       ...
       COALESCE(c.GerCdgMunSeap,-1) AS codibge7_escola,   
       ...
       tp.GerTpoAmbDsc AS tipo_ambiente,
       am.GerTpoAmbComDsc AS local_sala_anexa,     
       ...
       C2.GerCdgMunSeap AS codibge7_aluno,
       ...
       ROW_NUMBER() OVER (PARTITION BY m.GedAluCod ORDER BY m.GedMatCod DESC) AS Sequencial
       ...
       INNER JOIN GER.TBGERAMBIENTE am on am.GerLotCod = t.GerLotCod and am.GerAnoLetCod = t.GerAnoLetCod and am.GerAmbCod = t.GerAmbCod
       INNER JOIN GER.TBGERTPOAMBIENTE tp on tp.GerTpoAmbCod = am.GerTpoAmbCod
       ...
       AND m.GedMatDep = 0 
       AND M.GedMatUtiTraEsc = 1
       AND ISNULL(mr.GerMatRegFlgOpt,0) = 0
       AND ISNULL(mr.GerMatRegFlgOptIndependente,0) = 0
       ) A WHERE A.Sequencial = 1");
        ...
        $columns = array(
            'anobase', 'sre', 'tipo_dependencia', 'inep_escola', 'nome_escola', 'endereco_escola', 'numero_escola', 'complemento_escola', 'bairro_escola',
            'codibge7_escola', 'municipio_escola', 'estado_escola', 'cep_escola', 'ddd_telefone_escola', 'tipo_ambiente',

            //'cod_sala',

            'local_sala_anexa', 'tipo_ensino', 'sigla_tipo_ensino', 'modalidade',
            ...
            'dt_atualizacao', 'Sequencial'

        );
        // Salvar o CSV na pasta public
        ...
        foreach ($tasks as $task) {
            ...
            //  -- novas colunas
            $row['tipo_ambiente']                      = $task->tipo_ambiente;
            //    $row['cod_sala']                            = $task->cod_sala;
            $row['local_sala_anexa']                    = $task->local_sala_anexa;
            ...
            $row['Sequencial']                          = $task->Sequencial;
            ...
            fputcsv($file, array(
                $row['anobase'], $row['sre'], $row['tipo_dependencia'], $row['inep_escola'], $row['nome_escola'],
                $row['endereco_escola'], $row['numero_escola'], $row['complemento_escola'], $row['bairro_escola'], $row['codibge7_escola'],
                $row['municipio_escola'], $row['estado_escola'], $row['cep_escola'], $row['ddd_telefone_escola'],
                $row['tipo_ambiente'],
                //     $row['cod_sala'], 
                $row['local_sala_anexa'], $row['tipo_ensino'],
                ...
                $row['dt_atualizacao'], $row['Sequencial']
            ), $delimiter);
        }
        ...
        $originalPath = public_path('Transcolar.csv');

        // Diretório de destino dentro de 'public'
        $destinationDir = public_path('csv');

        // Caminho completo de destino
        $destinationPath = $destinationDir . '/Transcolar.csv';

        // Certifique-se de que o diretório de destino existe
        if (!file_exists($destinationDir)) {
            mkdir($destinationDir, 0777, true); // Cria o diretório, incluindo diretórios pai, se necessário
        }

        // Mover o arquivo
        if (file_exists($originalPath)) {
            if (rename($originalPath, $destinationPath)) {
                echo "Arquivo movido com sucesso para $destinationPath";
            } else {
                echo "Erro ao mover o arquivo.";
            }
        } else {
            echo "O arquivo original não foi encontrado.";
        }
        // dd($destinationPath);

        // Montar o link de download
        $downloadLink = asset('csv/Transcolar.csv');
        echo "Link para download: " . $downloadLink;
        // Verifique Fa conexão FTP tentando uma operação simples
        try {
            // Tente listar o diretório remoto
            Storage::disk('sftp')->files('/');

            // Enviar o arquivo via FTP
            $file_local = File::get($filePath);
            Storage::disk('sftp')->put($fileName, $file_local);

            // Enviar e-mail de notificação com a mensagem
            Mail::to('igorarrudabatista@gmail.com')->send(new CsvExported('O arquivo foi gerado e enviado via FTP.'));

            // Retornar sucesso sem conteúdo
            return response()->noContent();
        } catch (\Exception $e) {
            // Enviar e-mail de notificação informando que o FTP está fora do ar
            Mail::to('igorarrudabatista@gmail.com')->send(new CsvExported('O arquivo foi gerado, mas o FTP está fora do ar.'));
